// --- GLOBAL STATES (shared with all files) ---
...
function getProbabilityScore(w) {
  let counts = {};
  for (let c of w) counts[c] = (counts[c] || 0) + 1;
  let score = 1;
  for (let c in counts) score *= choose(letterFrequencies[c] || 0, counts[c]);
  return score;
}
    ...
    while(true) {
      ...
      if (statusEl) {
        if (totalBytes > 0) {
          ...
          statusEl.innerText = `Downloading Dictionary (${Math.round(loadedBytes/1024)} KB)`;
        }
      }
    }
    const statusEl = document.getElementById('loadingStatus');
...
function addFilter(mode) {
  ...
  const defaults = {
    length:                   { v1:'2',       v2:'8' },
    point_value:              { v1:'1',       v2:'50' },
    probability_order:        { v1:'1',       v2:'1000' },
    limit_probability_order:  { v1:'1',       v2:'100' },
    num_vowels:               { v1:'2',       v2:'4' },
    anagram_match:            { v1:'AEINRST', v2:'' },
    subanagram_match:         { v1:'AEINRST.', v2:'' },
    pattern_match:            { v1:'A...E',   v2:'' },
    begins:                   { v1:'',        v2:'' },
    ends:                     { v1:'',        v2:'' },
    includes:                 { v1:'',        v2:'' },
  };
...
function renderFilters(mode) {
  ...
  document.getElementById(mode === 'S' ? 'sStack' : 'qStack').innerHTML = arr.map(f => {
    ...
    const lbl = labels[f.type] || f.type.toUpperCase();
    return `
      <div class="f-item">
        <div style="display:flex; gap:6px; align-items:center; flex:1;">
          <button class="btn-not ${f.not ? 'active' : ''}" onclick="toggleNot('${mode}', ${f.id})">NOT</button>
          <span class="mono" style="color:var(--accent); font-size:11px;">${lbl}</span>
          ${isR
...
function matchFilters(w, arr) {
  for (let f of arr) {
    let m = true;
    const v1 = (f.v1 + '').trim().toUpperCase();
    switch (f.type) {
      case 'length':
        m = w.length >= parseInt(f.v1) && w.length <= parseInt(f.v2); break;
      case 'point_value':
        { let s = getWordScore(w); m = s >= parseInt(f.v1) && s <= parseInt(f.v2); break; }
      case 'begins': m = w.startsWith(v1); break;
      case 'ends':   m = w.endsWith(v1); break;
      case 'includes': m = [...v1].every(c => w.includes(c)); break;
      case 'num_vowels':
        { let vc = countVowels(w); m = vc >= parseInt(f.v1) && vc <= parseInt(f.v2); break; }
      case 'probability_order':
        { let r = probRankMap[w] || 9999999; m = r >= parseInt(f.v1) && r <= parseInt(f.v2); break; }
      case 'limit_probability_order':
        m = true; break; // handled post-filter
      case 'anagram_match':
        if (w.length !== v1.length) { m = false; break; }
        { const blanks = (v1.match(/\./g) || []).length;
          const rack = [...v1.replace(/\./g, '')]; let needed = 0;
          for (const c of w) { const i = rack.indexOf(c); if (i > -1) rack.splice(i,1); else needed++; }
          m = needed <= blanks; break; }
      case 'subanagram_match':
        { const blanks = (v1.match(/\./g) || []).length;
          ...
          m = needed <= blanks; break; }
      case 'pattern_match':
        try { m = new RegExp('^' + v1.replace(/\./g, '.').replace(/\*/g, '.*') + '$').test(w); }
        catch(e) { m = false; } break;
    }
    if (f.not) m = !m;
    if (!m) return false;
  }
...
function applyLimitFilters(res, filters) {
  let lf = filters.find(f => f.type === 'limit_probability_order');
  if (!lf) return res;
  res.sort((a, b) => {
    let d = (probRankMap[a] || 9999999) - (probRankMap[b] || 9999999);
    return d !== 0 ? d : (a < b ? -1 : 1);
  });
  let start = Math.max(0, parseInt(lf.v1) - 1);
  let end   = Math.max(0, parseInt(lf.v2));
  let sliced = res.slice(start, end);
  if (lf.not) { let ss = new Set(sliced); return res.filter(w => !ss.has(w)); }
  return sliced;
}
