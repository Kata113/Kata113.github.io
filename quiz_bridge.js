...
function mergeSessionIncorrect(source) {
  if (!source || typeof source !== 'object' || Array.isArray(source)) return;
  for (const [word, count] of Object.entries(source)) {
    const normalized = String(word).toUpperCase();
    const numericCount = Number(count);
    if (!normalized || !Number.isFinite(numericCount)) continue;
    sessionIncorrect[normalized] = Math.max(sessionIncorrect[normalized] || 0, numericCount);
  }
}
...
function loadSessionIncorrect() {
  ...
  try {
    ...
    mergeSessionIncorrect(raw ? JSON.parse(raw) : {});
  } catch(_) {}
}
...
function recordQuizHistory(result) {
  (result?.answers || []).forEach(answer => {
    if (answer.word && !quizHistory.some(item => item.word === answer.word)) {
      quizHistory.push({ word: answer.word, status: answer.status });
    }
  });
}
...
function saveCurrentZzq() {
  ...
  if (qFilters.length) {
    qFilters.forEach(f => {
      ...
      switch (f.type) {
        case 'length':
          lines.push(`     <condition type="Length" min="${escapeXmlAttr(f.v1)}" max="${escapeXmlAttr(f.v2)}"/>`); break;
        case 'point_value':
          lines.push(`     <condition type="Point Value" min="${escapeXmlAttr(f.v1)}" max="${escapeXmlAttr(f.v2)}"/>`); break;
        case 'num_vowels':
          lines.push(`     <condition type="Number of Vowels" min="${escapeXmlAttr(f.v1)}" max="${escapeXmlAttr(f.v2)}"/>`); break;
        case 'begins':
          lines.push(`     <condition type="Begins With" string="${escapeXmlAttr(f.v1)}" negated="${neg}"/>`); break;
        case 'ends':
          lines.push(`     <condition type="Ends With" string="${escapeXmlAttr(f.v1)}" negated="${neg}"/>`); break;
        case 'includes':
          lines.push(`     <condition type="Includes Letters" string="${escapeXmlAttr(f.v1)}" negated="${neg}"/>`); break;
        case 'probability_order':
          lines.push(`     <condition type="Probability Order" min="${escapeXmlAttr(f.v1)}" max="${escapeXmlAttr(f.v2)}"/>`); break;
        case 'limit_probability_order':
          ...
          lines.push(`     <condition int="2" type="Probability Order" max="${escapeXmlAttr(f.v2)}" bool="true" min="0"/>`); break;
        case 'anagram_match':
          lines.push(`     <condition type="Anagram Match" string="${escapeXmlAttr(f.v1)}" negated="${neg}"/>`); break;
        case 'subanagram_match':
          lines.push(`     <condition type="Subanagram Match" string="${escapeXmlAttr(f.v1)}" negated="${neg}"/>`); break;
        case 'pattern_match':
          lines.push(`     <condition type="Pattern Match" string="${escapeXmlAttr(f.v1)}" negated="${neg}"/>`); break;
      }
    });
  } else {
  ...
  const progressAttr = [
    ...
    `missed="${prog.totalMissed}"`,
    `incorrect="${prog.totalIncorrect}"`,
  ...
  if (hasBody) {
    ...
    if (correctW.length) {
      ...
      correctW.forEach(w => lines.push(`   <response word="${escapeXmlAttr(w)}"/>`));
      lines.push('  </question-correct-responses>');
    }
    if (incorrectW.length) {
      lines.push('  <incorrect-responses>');
      incorrectW.forEach(w => lines.push(`   <response word="${escapeXmlAttr(w)}" count="1"/>`));
      lines.push('  </incorrect-responses>');
    }
    if (missedW.length) {
      lines.push('  <missed-responses>');
      missedW.forEach(w => lines.push(`   <response word="${escapeXmlAttr(w)}" count="1"/>`));
      lines.push('  </missed-responses>');
    }
    lines.push(' </progress>');
  } else {
...
function loadXmlZzq(content) {
  ...
  if (prog) {
    ...
    const missed   = parseInt(prog.getAttribute('missed')            ?? '-1');
    const incorrect = parseInt(prog.getAttribute('incorrect')        ?? '-1');
    ...
    Module.restoreProgress(qIdx, correct, missed, incorrect, cq,
  ...
  while (walker.nextNode()) {
    ...
    if (m) {
      m[1].split(',').forEach(pair => {
        ...
        if (w) sessionIncorrect[w.toUpperCase()] = Math.max(
          sessionIncorrect[w.toUpperCase()] || 0,
          parseInt(c) || 1
        );
      });
      break;
